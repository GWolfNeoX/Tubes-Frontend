        ...
        <div class="register-form-container">
            <form id="registerForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="form-group">
                    <label for="uName">Inputkan nama:
                    </label>
                    <input type="text" name="uName" placeholder="nama lengkap" id="uName" class="form-control" v-model="registerObj.name" />
                    <p class="error-mess" v-if="errorObj.nameErr.length > 0">{{ errorObj.nameErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uEmail">Inputkan Email:
                    </label>
                    <input type="email" name="uEmail" placeholder="contoh@gmail.com" id="uEmail" class="form-control"
                        v-model="registerObj.email" />
                    <p class="error-mess" v-if="errorObj.emailErr.length > 0">{{ errorObj.emailErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uPass">inputkan password:
                    </label>
                    <input type="password" name="uPass" placeholder="inputkan password" id="uPass"
                        class="form-control" v-model="registerObj.pass" />
                    <p class="error-mess" v-if="errorObj.passErr.length > 0">{{ errorObj.passErr[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="uPassConfirm">Check password anda lagi:
                    </label>
                    <input type="password" name="uPassConfirm" placeholder="inputkan password anda lagi" id="uPassConfirm"
                ...
                <div class="form-group">
                    <label for="uPhone">Masukkan nomor HP:
                    </label>
                ...
                <div class="form-group">
                    <label for="uBirth">Inputkan Tanggal Lahir:
                    </label>
                ...
                <div class="form-group">
                    ...
                    <p>Punya akun? <router-link @click="scrollToTop()" to="/login">login</router-link>
                    </p>
                </div>
            </form>
        </div>
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.registerObj.name) {
                this.errorObj.nameErr.push("input nama diperlukan");
            }
            else {
                if (!/^[A-Za-z]+$/.test(this.registerObj.name.replace(/\s/g, ""))) {
                    this.errorObj.nameErr.push('Nama hanya bisa diisi dengan huruf');
                }
            }

            // Email validate
            if (!this.registerObj.email) {
                this.errorObj.emailErr.push("Input email diperlkukan");
            }
            else {
            ...
            if (!this.registerObj.pass) {
                this.errorObj.passErr.push('Password harus diisi');
            }
            else {
                if (!/[!@#$%^&*]/.test(this.registerObj.pass)) {
                    this.errorObj.passErr.push('Password setidak berisi 1 Spesial Karakter');
                }

                if (this.registerObj.pass.length < 6) {
                    this.errorObj.passErr.push('Password harus berjumlah 6 atau lebih karakter');
                }
            }

            // Confirm Pass validate
            if (!this.registerObj.confirm) {
                this.errorObj.confirmErr.push('Confirm password diperlukan');
            }
            else {
                if (this.registerObj.pass !== this.registerObj.confirm) {
                    this.errorObj.confirmErr.push('Confirm password harus sama dengan password');
                }
            }
            ...
            if (!this.registerObj.phone) {
                this.errorObj.phoneErr.push('input nomor hp diperlkan');
            }
            else {
                if (!this.registerObj.phone.startsWith('62')) {
                    this.errorObj.phoneErr.push('Nomor handphone harus dimulai dari 62');
                }

                if (this.registerObj.phone.length !=   13) {
                    this.errorObj.phoneErr.push('Nomor Handphone harus berisi setidaknya 13 digit');
                }

                if (!/[0-9]{11}/.test(this.registerObj.phone)) {
                    this.errorObj.phoneErr.push('Nomor Handhone hanya bisa berisi angka');
                }
            }

            // Birth validate
            if (!this.registerObj.birth) {
                this.errorObj.birthErr.push("Memasukan Tanggal Lahir Diperluan");
            }
            else {
                ...
                if (dateInput === "Invalid Date") {
                    this.errorObj.birthErr.push("Data tidak valid");
                }

                if (dateInput.getTime() < dateMin.getTime() || dateInput.getTime() > dateMax.getTime()) {
                    this.errorObj.birthErr.push("Hanya bisa berumur dibawah 150 tahun");
                }
            }

            // Gender validate
            if (!this.registerObj.gender) {
                this.errorObj.genderErr.push("Pilih gender");
            }
        },
